...
var RoomStatisticTableColumns = struct {
	RoomID string
	OnAir  string
}{
	RoomID: "room_statistics.room_id",
	OnAir:  "room_statistics.on_air",
}
...
func (r *roomStatisticR) GetRoom() *Room {
	if r == nil {
		return nil
	}
	return r.Room
...
var (
	...
	roomStatisticGeneratedColumns      = []string{}
)

type (
	// RoomStatisticSlice is an alias for a slice of pointers to RoomStatistic.
	// This should almost always be used instead of []RoomStatistic.
...
func (roomStatisticL) LoadRoom(e boil.Executor, singular bool, maybeRoomStatistic interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeRoomStatistic.(*RoomStatistic)
		if !ok {
			object = new(RoomStatistic)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeRoomStatistic)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeRoomStatistic))
			}
		}
	} else {
		s, ok := maybeRoomStatistic.(*[]*RoomStatistic)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeRoomStatistic)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeRoomStatistic))
			}
		}
	}
...
func RoomStatistics(mods ...qm.QueryMod) roomStatisticQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"room_statistics\".*"})
	}

	return roomStatisticQuery{q}
}
...
func (o *RoomStatistic) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(roomStatisticAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(roomStatisticPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert room_statistics, could not build conflict column list")
			}

			conflict = make([]string, len(roomStatisticPrimaryKeyColumns))
			copy(conflict, roomStatisticPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"room_statistics\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the RoomStatistic row exists.
func (o *RoomStatistic) Exists(exec boil.Executor) (bool, error) {
	return RoomStatisticExists(exec, o.RoomID)
}
