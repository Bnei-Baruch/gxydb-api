...
var CompositesRoomTableColumns = struct {
	CompositeID string
	RoomID      string
	GatewayID   string
	Position    string
}{
	CompositeID: "composites_rooms.composite_id",
	RoomID:      "composites_rooms.room_id",
	GatewayID:   "composites_rooms.gateway_id",
	Position:    "composites_rooms.position",
}
...
type compositesRoomR struct {
	Composite *Composite `boil:"Composite" json:"Composite" toml:"Composite" yaml:"Composite"`
...
func (r *compositesRoomR) GetComposite() *Composite {
	if r == nil {
		return nil
	}
	return r.Composite
...
func (r *compositesRoomR) GetGateway() *Gateway {
	if r == nil {
		return nil
	}
	return r.Gateway
...
func (r *compositesRoomR) GetRoom() *Room {
	if r == nil {
		return nil
	}
	return r.Room
...
var (
	...
	compositesRoomGeneratedColumns      = []string{}
)

type (
	// CompositesRoomSlice is an alias for a slice of pointers to CompositesRoom.
	// This should almost always be used instead of []CompositesRoom.
...
func (compositesRoomL) LoadComposite(e boil.Executor, singular bool, maybeCompositesRoom interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeCompositesRoom.(*CompositesRoom)
		if !ok {
			object = new(CompositesRoom)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeCompositesRoom)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeCompositesRoom))
			}
		}
	} else {
		s, ok := maybeCompositesRoom.(*[]*CompositesRoom)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeCompositesRoom)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeCompositesRoom))
			}
		}
	}
	...
	if singular {
		...
		args[object.CompositeID] = struct{}{}
	...
	query := NewQuery(
		qm.From(`composites`),
		qm.WhereIn(`composites.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (compositesRoomL) LoadGateway(e boil.Executor, singular bool, maybeCompositesRoom interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`gateways`),
		qm.WhereIn(`gateways.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func CompositesRooms(mods ...qm.QueryMod) compositesRoomQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"composites_rooms\".*"})
	}

	return compositesRoomQuery{q}
}
...
func (o *CompositesRoom) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(compositesRoomAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(compositesRoomPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert composites_rooms, could not build conflict column list")
			}

			conflict = make([]string, len(compositesRoomPrimaryKeyColumns))
			copy(conflict, compositesRoomPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"composites_rooms\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the CompositesRoom row exists.
func (o *CompositesRoom) Exists(exec boil.Executor) (bool, error) {
	return CompositesRoomExists(exec, o.CompositeID, o.RoomID, o.GatewayID, o.Position)
}
